...
from train_utils import run_epoch, LossCompute, NoamOpt, Transfer_Cnn6, Transfer_Cnn10, Transfer_Cnn14, create_data_paths_list
...
def run_finetuning_function(output_filename, model_type, model_load_path, model_save_path, pretrain):
    ...
    for epoch in range(100):
        model.train()
        loss, avg_loss, _, _, _, _, _, _, _ = run_epoch(model, 
        ...
        model.eval()
        with torch.no_grad():
            loss, _, val_acc, val_f1_score, true_val_f1_score, true_val_acc_score, _, _, _ = run_epoch(model, 
    ...
    with torch.no_grad():
        ...
        loss, _, valid_acc, valid_f1_score, true_valid_f1_score, true_valid_acc_score, _, valid_outputs, valid_targets = run_epoch(model,
                        LossCompute(model, None, pretrain=pretrain),
        ...
        loss, _, test_acc, test_f1_score, true_test_f1_score, true_test_acc_score, test_confusion_matrix, test_outputs, test_targets = run_epoch(model, 
                        LossCompute(model, None, pretrain=pretrain),
