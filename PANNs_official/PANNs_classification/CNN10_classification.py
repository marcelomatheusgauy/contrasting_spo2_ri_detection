...
validation_csv_file = '../alunos_finger_Tinder/SPIRA_Dataset_V2/metadata_eval_index_10.csv'
...
def run_epoch(model, loss_compute, data_paths, avg_loss=0, pretrain='pretrain', training=True, batch_size=16, extract_coeffs='both', min_frequency = 0.0, max_frequency=None, number_coeffs=128, mask_proportion=0., mask_consecutive_frames=7, mask_frequency_proportion=0., random_noise_proportion=0.0):
    ...
    return total_loss / (total_tokens), avg_loss, train_acc_avg, f1_score_avg, true_f1_score, true_acc_score, confusion_matrix
...
args['fmin']=0
args['fmax']=32000
args['model_type']="Transfer_Cnn14"
args['pretrained_checkpoint_path']="Cnn14.pth"
...
for epoch in range(10):
    ...
    model.eval()
    with torch.no_grad():
        loss, _, val_acc, val_f1_score, true_val_f1_score, true_val_acc_score, _ = run_epoch(model, 
