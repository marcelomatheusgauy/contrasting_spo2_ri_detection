...
class Cnn14(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        dropout_factor = 0.5#0.2

        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=dropout_factor, training=self.training)
        ...
        dropout_factor_2 = 0.75#0.5
        (x1, _) = torch.max(x, dim=2)
        x2 = torch.mean(x, dim=2)
        x = x1 + x2
        x = F.dropout(x, p=dropout_factor_2, training=self.training)
        x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=dropout_factor_2, training=self.training)
...
class Cnn10(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=dropout_1, training=self.training)
        x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=dropout_1, training=self.training)
        x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=dropout_1, training=self.training)
        ...
        x2 = torch.mean(x, dim=2)
        x = x1 + x2
        x = F.dropout(x, p=dropout_2, training=self.training)
        x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=dropout_2, training=self.training)
        clipwise_output = torch.sigmoid(self.fc_audioset(x))
        output_dict = {'clipwise_output': clipwise_output, 'embedding': embedding}
