...
from audiomae.util.patch_embed import PatchEmbed_new, PatchEmbed3D_new
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, mask_2d=True, use_custom_patch=False, **kwargs):
        ...
        depth=12
        mlp_ratio=4
        self.pre_head_FF_1 = nn.Linear(self.embed_dim, 10)
        self.mish = nn.GELU()
        self.dropout = nn.Dropout(p=0.3)
        self.pre_head_FF_2 = nn.Linear(10, 1)
    ...
    def forward(self, x, v=None, mask_t_prob=0.0, mask_f_prob=0.0):
        ...
        x = self.pre_head_FF_1(x)
        x = self.mish(x)
        #x = self.dropout(x)
        x = self.pre_head_FF_2(x)
        #x = self.reg_head(x)
        #x = self.head(x)
        return x
