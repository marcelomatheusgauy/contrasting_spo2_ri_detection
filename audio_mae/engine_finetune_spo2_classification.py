...
import audiomae.util.lr_sched as lr_sched
from spo2_classification.util.stat import calculate_stats, concat_all_gather
...
def evaluate(data_loader, model, device, dist_eval=False):
    ...
    tn, fp, fn, tp = stats[0]['confusion_matrix'].ravel()
    print("tp:", tp)
    print("tn:", tn)
    print("fn:", fn)
    print("fp:", fp)
    ...
    f1_score = np.mean([stat['f1_score'] for stat in stats])
    ...
    return {"mAP": mAP, "AP": AP, "acc": acc, "f1_score": f1_score}
