...
import audiomae.util.lr_decay as lrd
import audiomae.util.misc as misc
from audiomae.util.datasets import build_dataset
from audiomae.util.pos_embed import interpolate_pos_embed, interpolate_pos_embed_audio, interpolate_patch_embed_audio, interpolate_pos_embed_img2audio
from audiomae.util.misc import NativeScalerWithGradNormCount as NativeScaler

import models_vit_regression

from engine_finetune_spo2 import train_one_epoch, evaluate #, train_one_epoch_av, evaluate_av
from spo2_regression.dataset import SpiraDataset, DistributedWeightedSampler, DistributedSamplerWrapper
from timm.models.vision_transformer import PatchEmbed

from torch.utils.data import WeightedRandomSampler

from spo2_regression.utils.generic import load_config
...
def main(args):
    ...
    #do not fix seeds; repeat experiment to get mean result
    ...
    if not args.audio_exp:
        ...
        norm_stats = {'audioset':[-4.2677393, 4.5689974], 'k400':[-4.2677393, 4.5689974], 
                      'esc50':[-6.6268077, 5.358466], 'speechcommands':[-6.845978, 5.5654526], 'spira': [-6.7088156
,3.8913279], 'spira_2': [-6.6793118, 4.0290794]}
        target_length = {'audioset':1024, 'k400':1024, 'esc50':512, 'speechcommands':128, 'spira': 1024, 'spira_2': 1024} #spira should be 398 but model may not work with pretrained
        multilabel_dataset = {'audioset': True, 'esc50': False, 'k400': False, 'speechcommands': True, 'spira': False, 'spira_2': False} #spira entry should be false
        ...
        dataset_train = SpiraDataset(dataset_csv=args.data_train, audio_conf=audio_conf_train, folder_path=args.data_path,
                                        use_fbank=args.use_fbank, fbank_dir=args.fbank_dir, 
                                        roll_mag_aug=args.roll_mag_aug, load_video=args.load_video, mode='train')
        dataset_val = SpiraDataset(dataset_csv=args.data_eval, audio_conf=audio_conf_val, folder_path=args.data_path, use_fbank=args.use_fbank, fbank_dir=args.fbank_dir, 
                                        roll_mag_aug=False, load_video=args.load_video, mode='eval')
        dataset_test = SpiraDataset(dataset_csv=args.data_test, audio_conf=audio_conf_val, folder_path=args.data_path, use_fbank=args.use_fbank, fbank_dir=args.fbank_dir, roll_mag_aug=False, load_video=args.load_video, mode='eval')

    if 0==1: #args.distributed:#do not use distributed
    ...
    else:#will always use this path
    ...
    model_save_path='spo2_regression_finetune_mae.ckpt'
    ...
    model = models_vit_regression.__dict__[args.model](
    ...
    if args.finetune:
        ...
        if not args.eval:
            ...
            trunc_normal_(model.pre_head_FF_1.weight, std=2e-5)
            trunc_normal_(model.pre_head_FF_2.weight, std=2e-5)
            trunc_normal_(model.reg_head.weight, std=2e-5)
    ...
    if args.lr=="None":  # only base_lr is specified
    ...
    print(f"Start fold training for {args.epochs} epochs")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.first_eval_ep:
            ...
            print(f"MSE of the network on the {len(dataset_val)} validation audios: {test_stats['mse_error']:.8f}")
            print(f"MAE of the network on the {len(dataset_val)} validation audios: {test_stats['mae_error']:.8f}")
            print(f"R2 on the {len(dataset_val)} validation audios: {test_stats['r2']:.8f}")
            print(f"Pearson on the {len(dataset_val)} validation audios: {test_stats['pearson']:.8f}")
            if best_loss > test_stats["mse_error"]:
                best_loss = test_stats["mse_error"]
                torch.save({
                'model_state_dict': model.state_dict()
                }, model_save_path)
                
            print(f'Improved Best loss to: {best_loss:.8f}')
        else:
            test_stats ={'mse_error': 1e9}
            print(f'too new to evaluate!')

        #if log_writer is not None:
        #    log_writer.add_scalar('perf/mse_error', test_stats['mse_error'], epoch)
...
############################################################
    ...
    test_stats = evaluate(data_loader_test, model, device, args.dist_eval, args=args)
    print("Pearson of the network on the", len(dataset_test), "test fold audios:", test_stats['pearson'])
    ...
    print('========================================================================')
    print("TEST SUMMARY OF THE TEST SET")
    print('========================================================================')
    
    print("Test std:", test_stats["std_error"])
    print("Test MAE:",  test_stats["mae_error"])
    print("Test r2:", test_stats["r2"])
    print("Test pearson:", test_stats["pearson"])
    print("Test MSE:", test_stats["mse_error"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config_path', type=str,
                        default="spo2_regression/experiments/configs/exp-fase_1.json",
