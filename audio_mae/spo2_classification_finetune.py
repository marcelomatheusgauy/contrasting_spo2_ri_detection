...
from engine_finetune_spo2_classification import train_one_epoch, evaluate #, train_one_epoch_av, evaluate_av
from spo2_classification.dataset import SpiraDataset, DistributedWeightedSampler, DistributedSamplerWrapper
from timm.models.vision_transformer import PatchEmbed

from torch.utils.data import WeightedRandomSampler

from spo2_classification.utils.generic import load_config
...
,3.8913279], 'spira_2': [-6.6793118, 4.0290794]}
    ...
    model_save_path='spo2_classification_mae.ckpt'
    ...
    if args.eval:
        test_stats = evaluate(data_loader_test, model, device, args.dist_eval)
        with open('aps.txt', 'w') as fp:
            aps=test_stats['AP']
            aps=[str(ap) for ap in aps]
            fp.write('\n'.join(aps))
        print(f"Accuracy of the network on the {len(dataset_test)} test audios: {test_stats['acc']:.4f}")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.first_eval_ep:
            ...
            print(f"acc of the network on the {len(dataset_val)} validation audios: {test_stats['acc']:.4f}")
            print(f"F1-score on the {len(dataset_val)} validation audios: {test_stats['f1_score']:.4f}")
            if max_acc < test_stats["f1_score"]:
                max_acc = test_stats["f1_score"]
                torch.save({
                'model_state_dict': model.state_dict()
                }, model_save_path)
                
            print(f'Max F1-Score: {max_acc:.4f}')
        else:
            test_stats ={'acc': 0.0}
            print(f'too new to evaluate!')

        if log_writer is not None:
            log_writer.add_scalar('perf/f1_score', test_stats['f1_score'], epoch)
    ...
    test_stats = evaluate(data_loader_test, model, device, args.dist_eval)
    print(f"acc of the network on the {len(dataset_test)} test audios: {test_stats['acc']:.4f}")
    print(f"F1-score on the {len(dataset_test)} test audios: {test_stats['f1_score']:.4f}")
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config_path', type=str,
                        default="classification_ri/experiments/configs/exp-cv_Tinder.json",
