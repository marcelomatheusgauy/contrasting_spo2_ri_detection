...
class SpiraDataset(Dataset):
        ...
        if self.noise == True: # default is false
            ...
            if label == 1:
                for noise_index in range(num_patient_noise_files):
                    ...
                    max_val = float(noise_file.max().numpy())
                    if max_val == 0:
                        max_val = 1
                    reduct_factor = max_amp/max_val
    ...
    def _wav2fbank(self, filename, dat_index, label, filename2=None, dat_index2=None, label_2 = None):
        ...
        # mixup
            ...
            #print(filename)
            #print(dat_index)
            #print("waveform1", waveform1.shape)
            ...
            sample_rate = torchaudio.info(filename2).sample_rate
            ...
            #print(filename2)
            #print(dat_index2)
            #print("waveform2", waveform2.shape)
    ...
    def __getitem__(self, index):
        ...
        if random.random() < self.mixup: # for audio_exp, when using mixup, assume multilabel
            ...
            dat_file_path, dat_index = extract_index_from_path(datum['file_path'])
            ...
            mix_dat_file_path, mix_dat_index = extract_index_from_path(mix_datum['file_path'])
            mix_dat_file_path = self.folder_path+mix_dat_file_path
            
            # get the mixed fbank
            if not self.use_fbank:
                fbank, mix_lambda = self._wav2fbank(dat_file_path, dat_index, datum['insuficiencia_respiratoria'], mix_dat_file_path, mix_dat_index, mix_datum['insuficiencia_respiratoria'])
            else:
                fbank, mix_lambda = self._fbank(dat_file_path, mix_dat_file_path)
            # initialize the label
            label_indices = np.zeros(2)
            label_indices[datum['insuficiencia_respiratoria']] += mix_lambda
            label_indices[mix_datum['insuficiencia_respiratoria']] += (1.0-mix_lambda)
            label_indices = torch.FloatTensor(label_indices)
        # if not do mixup
        else:
            datum = self.data.iloc[index]
            ...
            dat_file_path = self.folder_path+dat_file_path
            label_indices = np.zeros(2)
            label_indices[datum['insuficiencia_respiratoria']] = 1.0
            label_indices = torch.FloatTensor(label_indices)
            if not self.use_fbank:
                fbank, mix_lambda = self._wav2fbank(dat_file_path, dat_index, datum['insuficiencia_respiratoria'])
            else:
                fbank, mix_lambda = self._fbank(dat_file_path)
        ...
        fbank = (fbank - self.norm_mean) / (self.norm_std * 2)
        # the output fbank shape is [time_frame_num, frequency_bins], e.g., [1024, 128]
        return fbank.unsqueeze(0), label_indices, datum['file_path']
